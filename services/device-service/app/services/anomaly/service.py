...
async def load_feature_windows_for_anomaly(
    ...
    from app.services.degradation.service import load_feature_windows_for_device

    return await load_feature_windows_for_device(db, tenant_id, device_id)
...
async def refresh_anomaly_baselines_for_device(
    ...
    windows = await load_feature_windows_for_anomaly(db, tenant_id, device_id)
    if not windows:
        return 0
