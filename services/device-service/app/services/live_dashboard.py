...
class LiveDashboardService:
    ...
    def _build_loss_overview(
        ...
        *,
        ...
        idle_kwh = self._round_float(state.today_idle_kwh if is_current_day and state is not None else 0.0)
        off_hours_kwh = self._round_float(state.today_offhours_kwh if is_current_day and state is not None else 0.0)
        overconsumption_kwh = self._round_float(
            state.today_overconsumption_kwh if is_current_day and state is not None else 0.0,
        )
        total_loss_kwh = self._round_float(state.today_loss_kwh if is_current_day and state is not None else 0.0)
        today_energy_kwh = self._round_float(state.today_energy_kwh if is_current_day and state is not None else 0.0)
        total_loss_cost_inr = (
    ...
    async def get_dashboard_summary(
        ...
        energy_service_attempted = plant_scope is None
    ...
    async def get_dashboard_bootstrap_summary(self, device_id: str, tenant_id: str) -> dict:
        ...
        loss_overview = self._build_loss_overview(
            state=state,
