...
async def test_get_monthly_calendar_overlays_current_day_with_live_dashboard_totals(monkeypatch):
    ...
    async def fake_totals(_tenant_id):
        return {
...
async def test_get_monthly_calendar_prefers_persisted_historical_costs_over_current_tariff(monkeypatch):
    engine = EnergyEngine(_SessionStub([
        SimpleNamespace(day=date(2026, 3, 30), energy_kwh=1.0, energy_cost_inr=6.5, version=2),
        SimpleNamespace(day=date(2026, 3, 31), energy_kwh=2.0, energy_cost_inr=14.0, version=3),
    ]))
    ...
    assert result["days"][29] == {"date": "2026-03-30", "energy_kwh": 1.0, "energy_cost_inr": 6.5}
    assert result["days"][30] == {"date": "2026-03-31", "energy_kwh": 2.0, "energy_cost_inr": 14.0}
