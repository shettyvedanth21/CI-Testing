...
async def test_monthly_calendar_filters_directly_by_fleet_tenant_id(session_factory, monkeypatch):
    ...
    async def fail_live_totals(_tenant_id: str | None):
        return None
    ...
    assert result["days"][19] == {"date": "2026-04-20", "energy_kwh": 2.0, "energy_cost_inr": 20.0}
    assert result["days"][20] == {"date": "2026-04-21", "energy_kwh": 1.5, "energy_cost_inr": 15.0}
...
async def test_monthly_calendar_returns_safe_zero_shape_without_tenant_scope(session_factory, monkeypatch):
    ...
    assert result["days"][19] == {"date": "2026-04-20", "energy_kwh": 0.0, "energy_cost_inr": 0.0}
