...
- multi-tenant async workload readiness
- workload fairness under burst
...
3. Operational queue/worker observability is not yet first-class across services.
...
Analytics, reporting, and waste analysis are now at the same queue/worker maturity level. Rules/notifications is the remaining gap.
...
1. Add per-tenant fairness controls to rules/notifications (admission caps, tenant-fair requeue, stale-ATTEMPTED claim fix).
2. Make "fairness knobs" explicit and environment-tunable across all services.
3. Expose queue depth and worker saturation as operational signals, not only internal implementation details.
...
Operational queue/worker observability and shared fairness module extraction.
